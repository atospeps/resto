...
class Tagger_Generic extends Tagger {
    ...
    private function getResults($tableName, $mapping, $footprint, $options) {
        ...
        return $this->query('WITH prequery AS (SELECT ' . $this->postgisGeomFromText($footprint) . ' AS corrected_geometry) SELECT ' . join(',', $propertyList) .  ' FROM prequery,' . $tableName . ' WHERE st_intersects(geom, corrected_geometry)' . $orderBy);
