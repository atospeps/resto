...
class Resto {
    ...
    private function authenticateBasic($token) {
        ...
        if (!empty($username) && !empty($password) && (bool) preg_match('//u', $username) && (bool) preg_match('//u', $password) && strrpos($username, '\'') === false) {
            try {
    ...
    private function toJSON($array) {
         ...
         * JSON-P case
         */
        $pretty = isset($this->context->query['_pretty']) && filter_var($this->context->query['_pretty'], FILTER_VALIDATE_BOOLEAN);
