...
class Functions_users {
	 ...
	 *
	 * @param string $identifier
	 *        	: can be email (or string) or integer (i.e. uid)
	 * @param string $password
	 *        	: if set then profile is returned only if password is valid
	...
	/**
	 * Save user profile to database i.e.
	...
	public function hasUserReachedLimitation($userprofile, $size) {
		...
		$query = 'SELECT items FROM usermanagement.orders WHERE email=\'' . pg_escape_string ($userprofile['email']) . '\'';
		$results = pg_fetch_all($this->dbDriver->query($query));
		$features = array ();
		foreach($results as $item) {
			$feature = json_decode($item['items']);
			$id = $feature[0]->{'id'};
			if ($features[$id]) {
				$features[$id] += 1;
			} else {
