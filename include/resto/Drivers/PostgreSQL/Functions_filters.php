...
class Functions_filters {
    ...
    private function prepareFilterQuery_date($model, $filterName, $filters) {
        ...
        return $model->getDbKey($model->searchFilters[$filterName]['key']) . ' ' 
             . $model->searchFilters[$filterName]['operation'] . ' \''
             . pg_escape_string($filters[$filterName]) . '\'';
    }
    ...
    private function prepareFilterQuery_distance($model, $filterName, $requestParams, $exclusion) {
        ...
        if (isset($requestParams['geo:lon']) && isset($requestParams['geo:lat'])) {
            ...
            if ($use_distance) {
                return 'ST_distance(' . $model->getDbKey($model->searchFilters[$filterName]['key']) . ', ST_GeomFromText(\'' . pg_escape_string('POINT(' . $requestParams['geo:lon'] . ' ' . $requestParams['geo:lat'] . ')') . '\', 4326)) < ' . $radius;
            }
            else {
